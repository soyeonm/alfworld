...
depths = [f.replace("/images/", "/depths/").replace('.png', '.p') for f in imgs]
depths = ['/'.join(d.split('/')[:-1]) + '/depth' + d.split('/')[-1] for d in depths]


hors = [f.replace("/images/", "/hors/").replace(".png", ".p") for f in imgs]
hors = ['/'.join(d.split('/')[:-1]) + '/horizon' + d.split('/')[-1] for d in hors]
